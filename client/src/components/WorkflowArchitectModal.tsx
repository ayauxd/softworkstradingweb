...
interface WorkflowArchitectModalProps {
  onClose: () => void;
}

const WorkflowArchitectModal = ({ onClose }: WorkflowArchitectModalProps) => {
  const { toast } = useToast();
  const [activeTab, setActiveTab] = useState<"none" | "chat" | "call">("none");
  const [showCallbackForm, setShowCallbackForm] = useState(false);
  const [chatInput, setChatInput] = useState("");
  const [messages, setMessages] = useState<Array<{from: string, text: string}>>([
    { from: "architect", text: "Hello! I'm here to help you automate your workflows. What specific process are you looking to improve?" }
  ]);
  ...
  const handleCallbackFormSubmit = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: "One of our workflow architects will call you shortly.",
    });
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg">
        <DialogTitle>Talk to a Workflow Architect</DialogTitle>
        <DialogDescription>
              ...
              {messages.map((message, index) => (
                <div key={index} className={`mb-4 ${message.from === "user" ? "text-right" : ""}`}>
                  <div className="font-medium text-navy dark:text-soft-white">
                    {message.from === "architect" ? "Workflow Architect" : "You"}
                  </div>
                  <div className={`
            ...
            {!showCallbackForm ? (
              ...
              <div>
                <p className="text-navy dark:text-soft-white text-center mb-4">
                  Our architects are currently assisting other clients. Leave your details for a callback.
                </p>
...
export default WorkflowArchitectModal;
